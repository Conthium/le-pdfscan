...
- **Document compare**: compares a left reference file with a right revised file directly in the browser. It accepts PDF, PNG, JPG, and WEBP, lets users select a separate comparison area on each document page, detects character-level changes from a PDF text layer when available, circles the related field in red, and exports the changed-page images as a ZIP.
